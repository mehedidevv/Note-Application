...
class DatabaseHelper(context: Context?) : SQLiteOpenHelper(context, DATABASE_NAME, null, VERSION) {
    ...
    //TODO Variable For Table Column
    companion object {
    ...
    fun updateData(id: Int, tittle: String?,description: String?): Boolean{
        val contentValues = ContentValues()
        ...
        sqLiteDatabase.update(TABLE_NAME, contentValues, "$COL_ID = ?", arrayOf(id.toString()))
        return  true
    }
